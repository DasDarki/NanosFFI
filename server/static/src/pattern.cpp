...
namespace hook
{
	...
	void set_base()
	{
		...
		set_base(reinterpret_cast<uintptr_t>(GetModuleHandle(nullptr)));
#elif defined(__linux__)
		set_base(reinterpret_cast<uintptr_t>(dlopen(nullptr, RTLD_LAZY)));
#endif
	}
	...
	void pattern::EnsureMatches(uint32_t maxCount)
	{
		...
		for (uintptr_t i = executable.begin(), end = executable.end() - maskSize; i <= end;)
		{
			...
			{
#if defined(_WIN32) || defined(_WIN64)
				i += max(ptrdiff_t(1), j - Last[ptr[j]]);
#elif defined(__linux__)
				i += std::max<ptrdiff_t>(1, j - Last[ptr[j] & 0xFF]);
#endif
